...
void RtuDateInit(void)
{
	...
	rtu.collecttime=   ReadDateEeprom(collecttimeAddress)<= 40?  ReadDateEeprom(collecttimeAddress) : 0;
...
void RtuSet(uint8_t * apc240RevDate, uint8_t apc240RecLen)
{
	...
	{
		if(apc240RevDate[1] == 0xff &&  apc240RevDate[2] == 0xff && apc240RevDate[3] >= 0 && apc240RevDate[3]<= 247 && apc240RevDate[4] <= 15
		&& apc240RevDate[5] <= 200 && (apc240RevDate[4] + apc240RevDate[5]) > 0 )
		{
			rtu.rtuId = apc240RevDate[3];
			rtu.chargeTime = apc240RevDate[4];
			rtu.batChargeTime = apc240RevDate[5];
			rtu.apc240_Channel = (Apc240_Channel)apc240RevDate[6];
//			rtu.collecttime=apc240RevDate[7];
			...
			WriteDateEeprom(collecttimeAddress, rtu.collecttime);
